...
const DART_EDGE_RUNTIME_NATIVE_ABI_VERSION: i32 = 8;
...
#[unsafe(no_mangle)]
pub extern "C" fn dart_edge_runtime_native_abi_version() -> i32 {
    DART_EDGE_RUNTIME_NATIVE_ABI_VERSION
}

#[unsafe(no_mangle)]
pub extern "C" fn dart_edge_runtime_start_server(
    host: *const c_char,
    port: i64,
    worker_count: i64,
    ...
    let compiled_manifest = match compile_manifest(&routes_json) {
        ...
        Err(error) => {
            eprintln!("dart_edge_runtime route manifest parse failed: {error}");
            return -1;
        }
    };
    ...
    *REQUEST_READY_CALLBACK.lock().unwrap() = Some(callback);
    ...
    let join_handle = thread::spawn(move || {
        ...
        runtime.block_on(async move {
            ...
            if let Err(error) = server.await {
                eprintln!("dart_edge_runtime transport server failed: {error}");
            }
        });
    });
    ...
    match ready_rx.recv() {
        ...
        Ok(Err(error)) => {
            eprintln!("dart_edge_runtime transport startup failed: {error}");
...
#[unsafe(no_mangle)]
pub extern "C" fn dart_edge_runtime_stop_server() {
    *REQUEST_READY_CALLBACK.lock().unwrap() = None;
    *COMPILED_ROUTES.write().unwrap() = Vec::new();
    *COMPILED_SCHEMAS.write().unwrap() = HashMap::new();
...
#[unsafe(no_mangle)]
pub extern "C" fn dart_edge_runtime_take_request(request_id: i64) -> *mut NativeTransportRequest {
    let mut pending = PENDING_REQUESTS.lock().unwrap();
...
#[unsafe(no_mangle)]
pub extern "C" fn dart_edge_runtime_free_request(value: *mut NativeTransportRequest) {
    if value.is_null() {
        return;
    }
...
#[unsafe(no_mangle)]
pub extern "C" fn dart_edge_runtime_send_response(
    request_id: i64,
    status: i32,
