...
import '../routes/handler_json_route_definition.dart';
...
///
/// Routers can be nested with [router] to build prefixed route groups while
/// still sharing the same underlying registry. Use [register] for reusable
/// route objects or the HTTP verb helpers for an inline Express-style API.
class Router<TServices> {
  Router({
    ...
    RouteRegistry<TServices>? routeRegistry,
       ...
       guards = List.unmodifiable(guards ?? const <Object>[]);
