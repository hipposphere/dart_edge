...
impl SharedSqlDatabaseAdapter {
    pub fn new(dialect: SharedSqlDialect, callbacks: SharedSqlCallbacks) -> Self {
        Self { dialect, callbacks }
    }

    pub fn run_migrations(&self) -> Result<(), DatabaseError> {
        let schema = match self.dialect {
            SharedSqlDialect::Postgres => POSTGRES_SCHEMA_SQL,
            SharedSqlDialect::Sqlite => SQLITE_SCHEMA_SQL,
        };
